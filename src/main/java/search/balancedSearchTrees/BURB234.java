...
/*This class implements the bottom up 2-3-4 trees as described in 3.3.28*/

class BURB234<Key extends Comparable<Key>, Value> {
	...
	private boolean is5node(Node n) {
		if (isRed(n.left) && isRed(n.right))
			if (isRed(n.right.left) || isRed(n.left.left))
				return true;
		return false;
	}
	...
	private Node put(Key k, Value v, Node n) {
		...
		// this variable is set to true if the current node n's child goes from black to red when the put function call returns
		boolean isFlip = false;
		int cmp = k.compareTo(n.k);
		if (cmp < 0) {
			isFlip = isRed(n.left);
			n.left = put(k, v, n.left); 
			isFlip = isRed(n.left) && !isFlip; // isFlip is true only when new color is red and old color is black
		}
		else if (cmp > 0) {
			isFlip = isRed(n.right);
			n.right = put(k, v, n.right); 
			isFlip = isRed(n.right) && !isFlip; // isFlip is true only when new color is red and old color is black
		}
		else n.v = v;
		if (isRed(n.right) && !isRed(n.left)) n = rotateLeft(n);
		if (isRed(n.left) && isRed(n.left.left)) n = rotateRight(n);
		// Only flip if this is a 5 node or if this is a newly created 4 node
		if (is5node(n) || (isFlip && isRed(n.left) && isRed(n.right))) n = flip(n);
		n.s = 1 + size(n.left) + size(n.right);
		return n;
	}
	...
	public static void main(String[] args) {
		...
		BURB234<Character, Integer> rbt = new BURB234<Character, Integer>();
		rbt.put(new Character('S'), new Integer(1));
		rbt.put(new Character('E'), new Integer(2));
